...
define_accessibleArea <- function(species_df, minBuff = 75000,
  ...
  temp_sf  <- st_as_sf(temp, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)
  ...
  shape <- getDynamicAlphaHull(
    x            = temp[, c("decimalLongitude", "decimalLatitude")],
    fraction     = 1,
    partCount    = 1,
    initialAlpha = 20,
  ...
  plotz  <- ggplot() +
    ...
    coord_sf(
      xlim = c(min(temp$decimalLongitude) - 10, max(temp$decimalLongitude) + 10),
      ylim = c(min(temp$decimalLatitude)  - 10, max(temp$decimalLatitude)  + 10)
    ) +
    theme_classic()

  if (saveImage) {
    ggsave(filename = paste0(imagedir, species_df$validName[1], ".png"), plot = plotz)
  } else {
  ...
  if (saveShapefile) {
    st_write(shape2_geo,
             dsn          = paste0(shapefiledir, species_df$validName[1], "_AA.shp"),
             delete_layer = TRUE)
  } else {
